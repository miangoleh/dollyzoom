...
class Pix2Pix4DepthModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(input_nc=2,output_nc=1,norm='none', netG='unet_1024', dataset_mode='template')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(2, 1, 64, 'unet_1024', 'none',
    ...
    def set_input(self, outer,inner,scale=True):
    ...
    def forward(self):
        ...
        # netout = self.netG(self.real_A)  # G(A)
        # mapper = netout[:,0,:,:]
        # confident = netout[:,1,:,:]
        # mapped = torch.mul(mapper,self.inner)
        #
        # self.fake_B = torch.mul(confident,mapped) + torch.mul(1-confident,self.outer)
        # confident = self.netG(self.real_A)  # G(A)
        # self.fake_B = torch.mul(confident,self.inner) + torch.mul(1-confident,self.outer)
